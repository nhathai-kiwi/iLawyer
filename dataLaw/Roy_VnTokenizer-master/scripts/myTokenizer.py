...
def reformat(mainText):
	mainText = unicode(mainText, "utf-8")
	sents_ = []
...
def mainFunction(mainText):
	...
	# 	print i
	return retWords
retWords = mainFunction("Kính gửi: Luật Sư Luật Minh Gia. Thưa Luật sư,Công ty em là công ty cổ phần nhưng các cổ đông sáng lập đều là tổ chức cả (8 cổ đông sáng lập đều là công ty khác) vậy ngay chỗ biên bản cuộc họp đại hội đồng cổ đông thì thành phần dự họp em không biết ghi như thế nào ạ mong Luật sư giải đáp với ạ? Nếu bên Luật sư có mẫu về biên bản cuộc họp đại hội đồng cổ đông mà các cổ đông sáng lập là tổ chức thì xin Luật sư cho em xin mẫu với ạ. Cám ơn Luật sư giúp đỡ ạ")
for i in retWords:
	print i
